...
__global__ void wmma_tf32_kernel(const float* a_row_major,
  ...
  for (int kk = 0; kk < k; kk += tile_k) {
    const float* a_tile = a_row_major + static_cast<size_t>(row) * k + kk;
    const float* b_tile = b_col_major + kk + static_cast<size_t>(col) * k;
...
void launch_wmma_tf32(const float* a_row_major,
  ...
  wmma_tf32_kernel<<<grid, block, 0, stream>>>(a_row_major, b_col_major, c_row_major, m, n, k);
}
